...
class main():
    def __init__(self) -> None:
        ...
        self.net_2class = feature_extraction_model_withdropout(n_channels=3,
                                                               output_ch=2,
                                                               start_n=8)
        if self.cuda:
            # put model on gpu
            self.net_2class = self.net_2class.cuda()
            self.net_2class.load_state_dict(
                torch.load(weight_path_2class)['model_state_dict'])
        else:
            self.net_2class.load_state_dict(
                torch.load(
                    weight_path_2class,
    ...
    def pad_input(self, input_img, patch_size=[32, 32]):
        ...
        if (hMod != patch_size[0] or wMod != patch_size[1]):
            output = np.pad(input_img, ((0, hMod), (0, wMod), (0, 0)),
                            'constant')
        else:
    ...
    def draw_center(self, image, c):
        ...
        cY = int(M["m01"] / M["m00"])
        # draw the countour number on the image
        image = cv2.circle(image, (cX, cY),
                           radius=1,
                           color=(0, 0, 255),
                           thickness=2)
        cv2.putText(image, "#{}".format('center'), (cX - 20, cY),
                    cv2.FONT_HERSHEY_SIMPLEX, 1.0, (255, 0, 0), 2)
        # return the image with the contour number drawn on it
        return image, cX, cY
    ...
    def find_center_point_text(self, dataFrame):
        temp = []
        dataFrame['center_x'] = dataFrame['x1'] + \
            (abs(dataFrame['x1']-dataFrame['x2'])/2)
        dataFrame['center_y'] = dataFrame['y1'] + \
            (abs(dataFrame['y1']-dataFrame['y2'])/2)

        for index, data in dataFrame.iterrows():
            updated_x, updated_y = self.rotate_point(
                (data['center_x'], data['center_y']))  # v1
        ...
        for (i, cnt) in enumerate(cnts):
            ...
            for index, data in dataFrame.iterrows():
                dist = cv2.pointPolygonTest(
                    cnt, (data['center_x'], data['center_y']), True)
                if dist > 0:
                    # positive ie point lies into polygon

                    if i not in index_conture_mapping_dict.keys():
                        image = self.fill_conture(
                            image, cnt, i, data['Features'])  # data['id']

                        index_conture_mapping_dict[i] = [{
                            'Features':
                            data['Features'],
                            'Dimension':
                            data['Dimension'],
    ...
    def find_split_precentages(self,
                               unique_class_list,
                               index_area_mapping_dict,
                               Quadrant_image_list=[]):
        quadrant_wise_dict = {}
            ...
            for class_index, pixel_count in np.asarray((unique, counts)).T:
                ...
                if class_index in unique_class_list and class_index != 0:
                    if index not in quadrant_wise_dict.keys():
                        print(
                            '(pixel_count)*100,index_area_mapping_dict[class_index] : ',
                            (pixel_count) * 100,
                            index_area_mapping_dict[class_index])
                        percrnt_value = (pixel_count)*100 / \
                            index_area_mapping_dict[class_index]
                        if percrnt_value > 0:
                            quadrant_wise_dict[index] = [{
                                class_index: {
                                    'pixel_count': pixel_count,
                                    'Percent %': '%.2f' % percrnt_value
                                }
                            }]
                    else:
    ...
    def rotate_img(self, image, org_image, angle):
        ...
        # imutils.rotate(image, -angle)#cv2.rotate(image,angle)#imutils.rotate_bound(image, angle)#
...
quadrant_wise_dict :  {
    ...
    def create_final_dataFrame(self, index_conture_mapping_dict,
                               quadrant_wise_dict):
        data_frame = pd.DataFrame(columns=[
            'index', 'Quadrant', 'Feature', 'Dimension', 'Total Area(Sq.Ft.)',
            'Area(Sq.Ft.)_in_Quadrant', 'Area(Sq.Ft.)_in_Quadrant %'
        ])
        final_dict = []
            ...
            for data_dict in data_list:
                ...
                if index in index_conture_mapping_dict:
                    ...
                    if len(feature_info) == 1:
                        # {'Features': 'M. BEDROOM', 'Dimension': '10\'11" X 16\'3"', 'Area (Sq. Ft.)': '177'}
                    ...
                    try:
                        temp['Area(Sq.Ft.)_in_Quadrant'] = int(
                            float(feature_info['Area (Sq. Ft.)']) *
                            float(percent) / 100)
                    except Exception as e:
                        # int( float(feature_info['Area (Sq. Ft.)'])*float(percent))
                        temp['Area(Sq.Ft.)_in_Quadrant'] = 'Manual'
    ...
    def draw_liners_text(self, image):
        ...
        image = cv2.line(image, (0, int(h / 2)), (w, int(h / 2)), (255, 0, 0),
                         thickness=2)
        image = cv2.line(image, (int(w / 2), 0), (int(w / 2), h), (255, 0, 0),
                         thickness=2)
        ...
        for q, point in q_dict.items():
            cv2.putText(image, q, point, font, fontScale, fontColor, thickness,
                        lineType)
    ...
    def Post_Processing(self, org_image, bin_image, dataFrame, roation_angle):
        ...
        cnts = cv2.findContours(bin_image, cv2.RETR_EXTERNAL,
        ...
        index_conture_mapping_dict, index_area_mapping_dict, class_image = self.map_class(
            bin_image, cntsSorted, dataFrame)

        cX, cY = self.find_center(cntsSorted.copy(), bin_image_copy)
        unique_class_list = np.unique(class_image)
        rotated_class_image, rotated_org_image = self.rotate_img(
            class_image, org_image, roation_angle)

        # print('class image np.asarray((unique, counts)).T : \n',np.asarray((unique, counts)).T)

        Quadrant_wise_image_list, Org_Quadrant_wise_image_list = self.split_image(
            rotated_class_image, rotated_org_image)
        quadrant_wise_info_dict = self.find_split_precentages(
            unique_class_list, index_area_mapping_dict,
            Quadrant_wise_image_list)
        final_df = self.create_final_dataFrame(index_conture_mapping_dict,
                                               quadrant_wise_info_dict)
        ...
        return final_df, (
            cX, cY
        ), Org_Quadrant_wise_image_list, rotated_org_image, rotated_class_image
    ...
    def Inferance(self, RGB_image, dataFrame, rotation_angle):
        ...
        final_rgb_mask_resized = cv2.resize(final_rgb, (org_w, org_h),
                                            0,
                                            0,
                                            interpolation=cv2.INTER_NEAREST)
        print('Resize Done.....', final_rgb_mask_resized.shape)
        final_df, center_point, Org_Quadrant_wise_image_list, rotated_org_image, rotated_class_image = self.Post_Processing(
            org_image, final_rgb_mask_resized, dataFrame, rotation_angle)
    ...
    def main(self, image, input_df, rotation_angle):
        ...
        final_df, center_point, Org_Quadrant_wise_image_list, rotated_org_image, rotated_class_image = self.Inferance(
            image, input_df, rotation_angle)
...
# dataFrame=pd.read_csv('2_png.csv')

# data=glob('D:/BizMetric/Project/Godrej/Unet/results/scripts/data/ocr/**')
